...
RSpec.describe 'タスク管理機能', type: :system do
  ...
  before do
    ...
    @task2 = FactoryBot.create(:second_task, title: 'task2', priority: 2, user_id: @task1.user.id)
    @task3 = FactoryBot.create(:second_task, title: 'task3',
      time_limit: '2020-02-28 10:00:00', priority: 0, user_id: @task1.user.id)
    visit root_path
  ...
  describe 'タスク詳細画面' do
     context '任意のタスク詳細画面に遷移した場合' do
       it '該当タスクの内容が表示されたページに遷移すること' do
         test3 = FactoryBot.create(:task, title: 'test3_title', content: 'test3_content', user_id: @task1.user.id)
